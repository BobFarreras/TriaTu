...
test.describe('recipes advanced', () => {
  test('edit, favorite, and rate recipe', async ({ page }) => {
    test.setTimeout(90_000);
    test.skip(!baseUrl || !serviceRoleKey, 'Missing SUPABASE_SERVICE_ROLE_KEY or NEXT_PUBLIC_SUPABASE_URL');

    const runId = Date.now();
    const email = `e2e_recipe_adv_${runId}@example.com`;
    const password = `E2E!${runId}a`;
    const recipeName = `E2E Recipe Adv ${runId}`;
    ...
    await test.step('favorite recipe on detail', async () => {
      await page.locator('[data-testid="recipe-favorite-button"]').last().click({ force: true });
    });

    await test.step('rate recipe from list', async () => {
      await page.goto('/recipes?mode=MINE');
      await page.waitForURL('**/recipes?mode=MINE');
      await page.locator(`[data-testid="recipe-card-${recipeId}"]`).scrollIntoViewIfNeeded();
      await page.locator(`[data-testid="recipe-rating-${recipeId}-star-5"]`).click();
    });

    await test.step('edit recipe', async () => {
      await page.goto(`/recipes/${recipeId}/edit`);
      await page.waitForURL(/\/recipes\/[0-9a-f-]{36}\/edit$/);

      await page.locator('[data-testid="recipe-title-input"]').fill(updatedName);
      await page.locator('[data-testid="recipe-tab-ingredients"]').click();
      await page.locator('[data-testid="recipe-ingredient-f2"]').click();

      await page.locator('[data-testid="recipe-tab-steps"]').click();
      await page.locator('[data-testid="recipe-step-input"]').fill('Pas 2: Serveix i gaudeix.');
      await page.locator('[data-testid="recipe-step-save"]').click();
      ...
      await expect(page.getByText('Poma verda')).toBeVisible();
      await expect(page.getByText('Pas 2: Serveix i gaudeix.')).toBeVisible();
    });
