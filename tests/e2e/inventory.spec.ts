...
test.describe('inventory', () => {
  ...
  test('consume shared inventory item', async ({ page }) => {
    test.setTimeout(90_000);
    test.skip(!baseUrl || !serviceRoleKey, 'Missing SUPABASE_SERVICE_ROLE_KEY or NEXT_PUBLIC_SUPABASE_URL');

    const runId = Date.now();
    const email = `e2e_inventory_consume_${runId}@example.com`;
    const password = `E2E!${runId}a`;
    const roomName = `E2E Inventory Consume ${runId}`;
    const productName = `E2E Inventory Consume Item ${runId}`;
    const productId = crypto.randomUUID();
    const externalId = `e2e-inv-consume-${runId}`;
