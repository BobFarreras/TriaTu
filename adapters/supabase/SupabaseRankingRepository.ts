...
export class SupabaseRankingRepository implements RankingRepository {
    ...
    async getTopPlayers(limit: number = 20): Promise<RankingEntry[]> {
        // 1. Consulta optimitzada: Ordenem per TOTAL a la BD
        const { data, error } = await this.supabase
            .from('user_leaderboard')
            .select('*')
            .order('total_score', { ascending: false }) // 🚀 La BD ordena, és més ràpid
            .limit(limit)
        ...
        // 2. Mapeig directe (sense re-calcular sumes ni re-ordenar)
        return data.map((row, index) => {
            const rank = index + 1; // La posició ve de l'ordre de la BD
            
            // Assegurem tipus numèrics (Postgres de vegades retorna strings per BigInt)
